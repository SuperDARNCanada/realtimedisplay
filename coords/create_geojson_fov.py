...
def make_geojson(site,rsep,nbeams,ngates,lagfr):
	...
	for beam in range(nbeams):
		for gate in range(ngates):
			...
			coords.append((FOV.lonFull[beam,gate],FOV.latFull[beam,gate]))
			coords.append((FOV.lonFull[beam+1,gate],FOV.latFull[beam+1,gate]))
			coords.append((FOV.lonFull[beam+1,gate+1],FOV.latFull[beam+1,gate+1]))
			coords.append((FOV.lonFull[beam,gate+1],FOV.latFull[beam,gate+1]))
			coords.append((FOV.lonFull[beam,gate],FOV.latFull[beam,gate]))
